...
#             ax2.axvline(x=21, c='k', linestyle='dashed', linewidth=0.5)
            ...
            if sd == 1:
                ...
                sd_type = 'one_third'
            plt.savefig('figure/distribution_plot/{}_{}_{}_{}_RNAup.png'.format(d_name,mut,sd_type,rc_type))
            plt.clf()
            plt.close()
            gc.collect()
        ...
        for rc_type in ['nor_readcount', 'nor_count']:
            ...
            for i in range(1,reg_seq_len+1): 
                ...
                if sd == 1:
                    ...
                    sd_type = 'one_third'
                plt.savefig('figure/pairing_ratio_plot/plot21/{}_{}_{}_{}_{}_RNAup.png'.format(str(i),d_name,mut,sd_type,rc_type), bbox_inches='tight')
                plt.clf()
                plt.close()
                gc.collect()
            # pairing ratio V2
            ...
            if sd == 1:
                ...
                sd_type = 'one_third'
            plt.savefig('figure/pairing_ratio_plot/plot_all/{}_{}_{}_{}_RNAup.png'.format(d_name,mut,sd_type,rc_type))
            plt.clf()
            plt.close()
            gc.collect()
