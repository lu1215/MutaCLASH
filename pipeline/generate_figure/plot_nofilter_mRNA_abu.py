...
#             print('{}: '.format(mut), len(del_three_third_data), len(del_two_third_data), len(del_one_third_data))
            ...
            for d1, d2 in zip(d1_list, d2_list):
                ...
                plt.tight_layout()
                plt.savefig('figure/abu_plot/tmp/{}_group{}_{}_{}_{}_with_mutation.png'.format(d_name, str(group), mut, d_n, score_type))
                plt.clf()
                plt.close()
                gc.collect()
...
#             print('{}: '.format(mut), len(del_three_third_data), len(del_two_third_data), len(del_one_third_data))
            ...
            for d1, d2 in zip(d1_list, d2_list):
                ...
                ax2.set_xlim(-1, 1)
                plt.tight_layout()
                plt.savefig('figure/abu_plot/tmp/{}_group{}_{}_{}_{}_with_mutation_cdf.png'.format(d_name, str(group), mut, d_n, score_type))
                plt.clf()
                plt.close()
                gc.collect()
